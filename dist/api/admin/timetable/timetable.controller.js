...
export const GetTimetableByDoctor = async (req, res, next) => {
    try {
...
export const UpdateTimetableById = async (req, res, next) => {
    try {
...
export const DeleteTimetableById = async (req, res, next) => {
    try {
        const timeID = req.params.id;
        if (!timeID)
            return res.status(404).json({ message: "Timetable not found" });
...
export const DeleteManyTimetables = async (req, res, next) => {
    try {
        const { timeIds } = req.body;
        const result = await prisma.timetable.deleteMany({ where: { timeID: { in: timeIds } } });
        if (result)
            return res.status(200).json({ message: "Delete successful", deletedCount: result.count });
    }
    catch (error) {
        next(error);
    }
