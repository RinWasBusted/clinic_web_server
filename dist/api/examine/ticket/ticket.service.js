...
class EnterTicketService {
    ...
    async findAppointment(payload) {
        ...
        if (!roomID)
            throw new NotFoundError("Appointment data is incomplete");
        return {
            note,
            appointmentID,
            patientID,
    ...
    async findLatestOrderNum(roomID) {
    ...
    async getWaitingList(query) {
        ...
        // Paginate service
        const waitingList = await paginateQuery(prisma.enterTicket, where, { page, limit }, {
            include,
            orderBy,
        });
        // Convert checkIn to local time (UTC + 7) for API response
        if (waitingList.data && Array.isArray(waitingList.data)) {
            waitingList.data.forEach((ticket) => {
    ...
    async getEnterTicketByID(ticketID) {
    ...
    async updateEnterTicket(ticketID, status) {
    ...
    async getCurrentTicket(roomID) {
        const { start, end } = getLocalStartAndEndOfDayInUTC();
        function renderTicketViewToDisplay(ticket) {
            if (!ticket)
                return null;
            return {
        ...
        return renderTicketViewToDisplay(ticketInService);
    }
}
