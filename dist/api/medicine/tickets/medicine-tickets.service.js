...
export const getMedicineTicketsService = async (date) => {
...
export const createMedicineTicketService = async (prescriptionDisplayID, accountID) => {
...
export const updateMedicineTicketStatusService = async (ticketId, status) => {
...
export const dispenseMedicineTicketService = async (ticketId, accountID) => {
    return prisma.$transaction(async (tx) => {
        ...
        const insufficientMedicines = ticket.prescription.details.filter((detail) => detail.medicine.quantity < detail.quantity);
        if (insufficientMedicines.length > 0) {
            const shortageMessage = insufficientMedicines
                .map((detail) => `${detail.medicine.medicineName} (cần ${detail.quantity}, còn ${detail.medicine.quantity})`)
                .join(", ");
            throw new MedicineTicketServiceError(`Insufficient stock: ${shortageMessage}`, 400);
        }
        const prescriptionCode = ticket.prescription.prescriptionDisplayID ?? ticket.prescription.prescriptionID;
        const exportNote = `Xuat thuoc cho don thuoc co ma ${prescriptionCode}`;
        const exportValue = ticket.prescription.details.reduce((total, detail) => total + Number(detail.medicine.price) * detail.quantity, 0);
...
//# sourceMappingURL=medicine-tickets.service.js.map