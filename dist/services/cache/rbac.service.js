...
class RBACService extends CacheService {
     ...
     */
    async cacheRole(roleName, permissionList) {
        if (!this.isConnected) {
     ...
     */
    async deleteCachedRole(roleName) {
        if (!this.isConnected) {
            console.warn("Redis server is not connected. Cache operation failed.");
            return;
        }
     ...
     */
    async checkCachedRoleExists(roleName) {
        if (!this.isConnected) {
            console.warn("Redis server is not connected. Cache operation failed.");
            return false;
        }
